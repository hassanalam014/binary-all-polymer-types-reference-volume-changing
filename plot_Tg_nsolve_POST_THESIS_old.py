...
def straightline_interpolation(y1,y2,x1,x2,x=0):

	# x is the criterion that is required to be x = 0.
	# x1 = criterion1 and x2=criterion2
	# y1 = T1 and y2= T2

	y = ((y2-y1)/(x2-x1))*(x-x1)+y1

	# print 'criteion1 =',x1,'criteion2 =',x2,'T1/P1 =',y1,'T2/P2 =',y2,'criterion =',x,'T/P =',y
	prPurple('criteion1={}, criteion2={}, T1/P1={}, T2/P2={}, criterion={}, T/P={}'.format(x1,x2,y1,y2,x,y))

	return y

def self_bisect_Tg_old(T1,T2,P,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward):

	number_of_trails = 1

	while number_of_trails<=5:
		
		print 'number_of_trails:', number_of_trails
		
		number_of_trails = number_of_trails+1
		...
		if abs(criterion2) <=1E-5:
			Tg = T2
			break

		if (criterion1<0 and criterion2<0) or (criterion1>0 and criterion2>0):
			Tg = 0.0
			print 'Failed: Sign of both criterions are the same'
			break

		if (criterion1>0 and criterion2<0) or (criterion1<0 and criterion2>0):
			T_trail = straightline_interpolation(T1,T2,criterion1,criterion2)
			criterion_trail = Find_Tg_Bisect_xS_infty(T_trail,P,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward)
			
			if (criterion_trail>0 and criterion1>0 and criterion2<0) or (criterion_trail<0 and criterion1<0  and criterion2>0):
				T1 = T_trail
				criterion1 = criterion_trail
			...
			elif (criterion_trail>0 and criterion2>0 and criterion1<0) or (criterion_trail<0 and criterion2<0 and criterion1>0):
				T2 = T_trail
				criterion2 = criterion_trail
			...
			if abs(criterion1)<=1E-5:
				# Tg = straightline_interpolation(T1,T2,criterion1,criterion2)
				Tg = T1
				print 'Hurry! Tg found to be:', Tg
				break
			elif abs(criterion2)<=1E-5:
				# Tg = straightline_interpolation(T1,T2,criterion1,criterion2)
				Tg = T2
				print 'Hurry! Tg found to be:', Tg
				break
	...
	while number_of_trails<=1:
		...
		if (criterion1>0 and criterion2<0) or (criterion1<0 and criterion2>0):
			...
			if (criterion_trail>0 and criterion1>0 and criterion2<0) or (criterion_trail<0 and criterion1<0  and criterion2>0):
				T1 = T_trail
				criterion1 = criterion_trail
				prRed('T1_trail={} and T2= {}'.format(T1,T2))
				
			elif (criterion_trail>0 and criterion2>0 and criterion1<0) or (criterion_trail<0 and criterion2<0 and criterion1>0):
				T2 = T_trail
				criterion2 = criterion_trail
				prRed('T1={} and T2_trail= {}'.format(T1,T2))
...
def self_bisect_Pg_new(P1,P2,T,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward):

	number_of_trails = 1
	dP = abs(P1-P2)/10
	
	while number_of_trails<=10:
		...
		if (criterion1>0 and criterion2<0) or (criterion1<0 and criterion2>0):
			...
			# P_trail = straightline_interpolation(P1,P2,criterion1,criterion2)
			if P1>P2:
				P1_trail = P1 - dP
				# P2_trail = P2 + dP
			elif P2>P1:
			...
			criterion1_trail = Find_Pg_Bisect_xS_infty(P1_trail,T,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward)
			# criterion2_trail = Find_Pg_Bisect_xS_infty(P2_trail,T,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward)
			
			if (criterion1_trail>0 and criterion1>0 and criterion2<0) or (criterion1_trail<0 and criterion1<0  and criterion2>0):
				P1 = P1_trail
				criterion1 = criterion1_trail
				prGreen('P1_trail={} and P2= {}'.format(P1,P2))
			
			elif (criterion1_trail<0 and criterion1>0 and criterion2<0) or (criterion1_trail>0 and criterion1<0  and criterion2>0):
				dP = dP/5
				prYellow('P1_trail={} and P1= {}'.format(P1_trail,P1))
				prYellow('dP = {} and criterion1_trail = {}'.format(dP,criterion1_trail))

			# if (criterion2_trail>0 and criterion2>0 and criterion1<0) or (criterion2_trail<0 and criterion2<0 and criterion1>0):
			# 	P2 = P2_trail
			# 	criterion2 = criterion2_trail
			# 	prRed('P1={} and P2_trail= {}'.format(P1,P2))

			if dP<=1E-3 or abs(criterion1)<1E-2:
				# Pg = straightline_interpolation(P1,P2,criterion1,criterion2)
				Pg = P1
				print 'Hurry! Pg found to be:', Pg, 'and criterion'
				break
			# elif abs(P2)<=1E-5:
			# 	# Pg = straightline_interpolation(P1,P2,criterion1,criterion2)
			# 	Pg = P2
			# 	print 'Hurry! Pg found to be:', Pg
			# 	break

	return Pg

def self_bisect_Pg_old(P1,P2,T,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward):
	...
	while number_of_trails<=10:
		...
		if (criterion1>0 and criterion2<0) or (criterion1<0 and criterion2>0):
			...
			P_trail = straightline_interpolation(P1,P2,criterion1,criterion2)
			criterion_trail = Find_Pg_Bisect_xS_infty(P_trail,T,S_infty,Mp,Ms,g,epsilon_p,x,zeta,delta,Ppstar,Tpstar,Rpstar,Psstar,Tsstar,Rsstar,Kier,Hassan,Condo,Hassan_Var_Vol,forward,backward)
			
			if (criterion_trail>0 and criterion1>0 and criterion2<0) or (criterion_trail<0 and criterion1<0  and criterion2>0):
				P1 = P_trail
				criterion1 = criterion_trail
				prRed('P1_trail={} and P2= {}'.format(P1,P2))
				# P2 = (P1+P2)/2
				# if first_time == True:
				# 	first_time = False
				# 	if P2>P1:
...
def binaryPhaseEquilibriumCHV(P,T,Mp,Ms,**kwargs):
	...
	guess1 = npy.array([0.85,0.95,0.7,0.5,0.99,0.999,0.10,0.30,0.01,0.05])
	...
	for i in range(len(guess1)):
		...
		if phip0.real>0.0 and abs(phip0.imag)<=10E-3:
			...
			break				#Break it because PMMA has only one possible solution.
		else:
			# phip0 = 0.0
			pass
...
P=npy.linspace(0.101325,6,20)
T=npy.linspace(250,360,20)
